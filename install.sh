...
install_dependencies() {
    ...
    apt-get install -y ca-certificates curl jq unzip wget nano htop pwgen \
        fail2ban ufw util-linux lsb-release openssl
...
create_server_users() {
    local count username password confirmation i
    while true; do
...
resolve_downloads() {
    ...
    if (( PC == 1 )); then
        POWCACHE_URL="$(github_latest_asset "$BOOTSTRAP_REPO" 'powcache\.dat$')" ||
